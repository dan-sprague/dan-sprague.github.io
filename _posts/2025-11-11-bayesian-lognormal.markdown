...
- **Bayesian Hierarchical Model**: A statistical method is proposed that better estimates the underlying distirbution and uncertainty for lognormal data.
- **Regularization benefit**: The common sense hierarchical priors prevent extreme parameter estimates that commonly occur with small sample maximum likelihood estimation, particularly for scale parameters
- **Average KL divergence**: Improved modeling of lognormal data uncertainty causes large (approximately 90% reduction) and significant decrease in average KL divergence in simulation experiment.
